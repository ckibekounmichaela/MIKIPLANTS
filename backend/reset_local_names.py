...
NOMS = {
    ...
    "Artocarpus altilis":                   "fruit de l arbre a pain",
...
def reset():
    ...
    print()
    print("=" * 55)
    print("  MikiPlants -- Reset noms courants CI")
    print("=" * 55)
    print()

    for sci_name, nom_ci in NOMS.items():
        plant = db.query(Plant).filter(
            Plant.scientific_name == sci_name
        ).first()
        if plant:
            plant.local_name = nom_ci
            updated += 1
            print(f"  [OK] {plant.name:35s} -> {nom_ci}")
        else:
    ...
    print()
    print("=" * 55)
    print(f"  Mis a jour : {updated} plantes")
    if not_found:
